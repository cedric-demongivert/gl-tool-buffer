...
export class GLTVertexBuffer {
   ...
   * @param [littleEndian=GLTEndianess.IS_LITTLE_ENDIAN] - True for storing values in little-endian, false otherwise.
   */
  public constructor(buffer: ArrayBuffer, size: number = 0, littleEndian: boolean = GLTEndianess.IS_LITTLE_ENDIAN) {
    this._buffer = buffer
  ...
  public set capacity(capacity: number) {
    if (capacity < 0) {
      throw new Error('A GLTVertexBuffer capacity can\'t be negative.')
    }
  ...
  public set size(newSize: number) {
    if (newSize < 0) {
      throw new Error('A GLTVertexBuffer size can\'t be negative.')
    }
   ...
   */
  public copy(source: GLTVertexBuffer, sourceOffset: number = 0, destinationOffset: number = 0, size: number = source.size): void {
   ...
   */
  public concat(...others: GLTVertexBuffer[]): void {
  ...
  public equals(other: any): boolean {
    ...
    if (other instanceof GLTVertexBuffer && other.size === this.size) {
   ...
   */
  public clone(): GLTVertexBuffer {
    return new GLTVertexBuffer(this._buffer.slice(0), this._size)
  }
 ...
 * 
 */
export namespace GLTVertexBuffer {
  /**
   * Create a new empty vertex buffer with an initial capacity.
   *
   * @param [capacity = 16] - Initial capacity of the created buffer.
   * @param [littleEndian = GLTEndianess.IS_LITTLE_ENDIAN] - GLTEndianess of the created buffer.
   *
   * @return The created buffer.
   */
  export function empty(capacity = 16, littleEndian = GLTEndianess.IS_LITTLE_ENDIAN) {
    return new GLTVertexBuffer(new ArrayBuffer(capacity), 0, littleEndian)
  }
   ...
   * 
   */
  export function copy(toCopy: GLTVertexBuffer): GLTVertexBuffer
  /**
   * 
   */
  export function copy(toCopy: GLTVertexBuffer | undefined | null): GLTVertexBuffer | undefined | null
  export function copy(toCopy: GLTVertexBuffer | undefined | null): GLTVertexBuffer | undefined | null {
